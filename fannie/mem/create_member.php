...
if (isset($_POST['submitted'])) { // If the form has been submitted, check the data and create the record.
	...
	if ((isset($_POST['ps'])) && (isset($_POST['ss']))) { // Both Primary and Secondary. 
		...
		} elseif (($_POST['ps_discount'] > 25) || ($_POST['ss_discount'] > 25)) {
		...
		if ((empty($_POST['ps_first_name'])) && (empty($_POST['ps_last_name']))) {
			$errors[] = 'You left first and last names blank.';
		} else {
...
$query3 = "SELECT max(cardno) AS max FROM custdata WHERE cardno < 9000";
...
// Show the form.
	...
	<p>Discount: <input type="text" name="ps_discount" size="3" maxlength="2" value="0" />%</p>
