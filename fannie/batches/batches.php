...
elseif(!isset($batchID)) { echo($_POST['batchID'] . "wtf");
...
while($selBItemsW = mysql_fetch_array($selBItemsR))  {
	$upc = $selBItemsW['upc'];
	$field = 'sale'.$upc;
	$del = 'del'.$selBItemsW[1];
	...
	if(!$selBItemsW['upc']) {
		...
		echo "<td align=center>".$selBItemsW['upc']."</td><td>".$selBItemsW['description']."</td>";
	}
	echo "<td align=right>".$selBItemsW['normal_price']."</td><td align=center><input type=text name=$field value='" .  money_format('%n',$selBItemsW['salePrice']) . "' size=8></td>";
	if(!$selBItemsW['upc']) {
	echo "<input type=hidden name=upc value='$selBItemsW[1]'>";
	}
	else {
