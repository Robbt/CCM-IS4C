...
#mysql -u root -ppassword < /pos/fannie/scripts/batchUpdate
...
mysql -u root -ppassword is4c_log -e "INSERT INTO dlog_"$(date +%Y)" (SELECT * FROM dtransactions)"

if [ $? == 0 ]
then
mysql -u root -ppassword is4c_log -e "TRUNCATE TABLE dtransactions"
echo "`$date` - dtransactions archived and truncated." >> /pos/fannie/logs/nightlylog.txt
else
mysqldump -u root -ppassword is4c_log dtransactions > /pos/archives/dtransactions_`$date`.sql
mysqldump -u root -ppassword is4c_log dlog_'$year' > /pos/archives/dlog_`$(date +%Y)`_`$date`.sql
echo "`$date` - ERROR - Archival failed. Backed up transaction tables." >> /pos/fannie/logs/nightlylog.txt
fi

mysqldump -u root -ppassword is4c_op custdata products | mysql -u is4cserver -h 10.22.33.1 opdata
...
mysqldump -u root -ppassword is4c_op  custdata products | mysql -u is4cserver -h 10.22.33.2 opdata
...
mysqldump -u root -ppassword is4c_op custdata products | mysql -u is4cserver -h 10.22.33.3 opdata
