...
*********************************************************************************/
//PLC 20080408 Added new variable to track cost history
...
$query = "UPDATE products 
	...
	where upc ='$upc'";
echo $query;
//PLC 20080408 Added Error Checking
if (!mysql_query($query)) {
	die("Products:" . mysql_error());
	exit;
} else {
	//first lookup id from products
	$pid = mysql_result(mysql_query(sprintf("select id from products where upc = %d",$upc)),0);
	$historysql = sprintf("insert into products_price_history (id,upc,normal_cost) values (%d,%d,%f)",$pid,$upc,$_POST['currentprice']);
...
echo "<table border=0>";
        ...
        echo "<td><b>Price</b></td><td>$".$row['normal_price']."</td></tr>";
	echo "<tr><td><b>Wholesale Cost</b></td><td>$".$row['wholesale_cost']."</td></tr></table>";
        echo "<table border=0><tr>";
        echo "<th>Dept<th>subDept<th>Tax<th>FS<th>Scale<th>QtyFrc<th>NoDisc<th>inUse<th>deposit</b>";
        echo "</tr>";
        echo "<tr>";
        $dept=$row['department'];
		...
		$row2a['subdept_name'];
		echo " </td>";
		echo "<td align=center><input type=checkbox value=1 name=tax";
                if($row["tax"]==1){
                        echo " checked";
                }
