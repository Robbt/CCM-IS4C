...
// ccm-rle      Added sales tax for CCM
		$taxes = taxes($table,$date1a,$date2a);
...
// the original		$net = $gross + $hash + $totalDisc + $coupons + $strchg + $RA;
			$net = $gross + $totalDisc + $taxes;



/* Replacing the Inventory Sales and Non Inventory Sales with a simple report..
That just adds up total sales */
if ($gross == 0 || !$gross) $gross = 1; //to prevent division by 0 or division by null in the query below
		...
		$invtotalsQ = "SELECT d.department,t.dept_name,ROUND(sum(d.total),2) AS total,ROUND((SUM(d.total)/$gross)*100,2) as pct
			...
			AND date(d.datetime) >= '$date1a' AND date(d.datetime) <= '$date2a' 
...
//		$gross = 0;
		...
		$noninvtotalsQ = "SELECT d.department,t.dept_name,ROUND(sum(total),2) as total, count(d.datetime) AS count
			...
			AND d.emp_no <> 9999
			GROUP BY d.department, t.dept_name";
//ccm-rle removed store charges coupons rcvd/accts hash total and the like
                echo "<h2>Income</h2>\n
			...
			<tr><td>Sales tax</td><td align=right>+".money_format('%n',$taxes)."</td></tr>\n
...
/*  ccm-rle removed the following and replaced it with the above		
...
*/			
		...
		echo '</p>';
// ccm-rle      Non-Inventory Departments not used by CCM
...
*/
	...
	if(isset($discounts)) {
                echo "<h2>Membership & Discount Totals</h2>\n
                        <table border=0>\n<font size=2>\n
                        <tr><td>Members total</td><td align=right>".money_format('%n',$members_total)."</td></tr>\n
                        <tr><td>Non-Members total</td><td align=right>".money_format('%n',$nonmembers_total)."</td></tr>\n
	...
	*/
		...
		$nodiscountQ = "SELECT COUNT( DISTINCT trans_no, emp_no, register_no, datetime )
				FROM is4c_log.$table 
				WHERE DATE(datetime) >= '$date1a' AND DATE(datetime) <= '$date2a'
				AND `upc` = 'DISCOUNT'
	                        AND trans_status <> 'X' AND emp_no <> 9999 AND card_no <> 99999
				AND `unitPrice` = 0";

                $nodiscountR = mysql_query($nodiscountQ);
                $row = mysql_fetch_row($nodiscountR);
                $nodiscount = $row[0];

                echo '<b></p>Non-Discounted Transactions:</b> '.$nodiscount .'</br>';

                $discountedQ = "SELECT COUNT( DISTINCT trans_no, emp_no, register_no, datetime )
                                FROM is4c_log.$table 
                                WHERE DATE(datetime) >= '$date1a' AND DATE(datetime) <= '$date2a'
                                AND `upc` = 'DISCOUNT'
                                AND trans_status <> 'X' AND emp_no <> 9999 AND card_no <> 99999
                ...
                $discountedR = mysql_query($discountedQ);
                $row = mysql_fetch_row($discountedR);
                $discounted = $row[0];
                echo '<b>Discounted Transactions:</b> '.$discounted;
