    ...
    class chainedSelectors
    {
        ...
        */
		...
		$selected_index = 0;
            ...
            $sourceSelector = "document." . $this->names[CS_FORM] . "." . $this->names[CS_FIRST_SELECTOR];
            $targetSelector = "document." . $this->names[CS_FORM] . "." . $this->names[CS_SECOND_SELECTOR];
            ...
            */
            ...
            {
			print("//check:" . $sourceValue .":" . $selected_item['department'] . ":" . $selected_item['subdept'] . ":" . ($selected_item['subdept'] - ($sourceValue * 100)) . "\n");
				//if($sourceValue == $selected_item['department']) {		// [12] = Department number
				//	$selected_index = $selected_item['subdept'];		// [28] = subdept. #
                ...
                print("\tif(" . $sourceSelector .
                    ".options[whichSelected].value == " . "'$sourceValue')\n");
                print("\t{\n");
                ...
                foreach($this->data as $row)
                {
                    if($row[0] == $sourceValue)
                    {
                        ...
                        print("\t\t" . $targetSelector .".options[$count].value = '$optionValue';\n");
                        print("\t\t" . $targetSelector .".options[$count].text = '$optionLabel';\n\n");
			//if($count + 1 == $selected_index){
			if ($sourceValue == $selected_item['department'] && $optionValue == $selected_item['subdept']) {
				print("\t\t" . $targetSelector .".options[$count].selected = true;\n");
			}
                        $count++;
                    }
                }
        ...
        function printSelectors($item_selected)
        {
            ...
            */
            ...
            foreach($this->uniqueChoices as $key=>$value)
            {
                ...
                if($key == $item_selected['department']) 	// [12] = department
                {
