...
echo '<HEAD><script src="../src/CalendarControl.js" language="javascript"></script>
	...
	if (typeof(mylink) == "string")
	   ...
	   href=mylink.href;
	window.open(href, windowname, "width=600,height=600,scrollbars=yes,menubar=no,location=no,toolbar=no,dependent=yes");
	return false;
	}
	//-->
	</SCRIPT>
...
//			echo "$query";
			...
			while ($row = mysql_fetch_array ($result, MYSQL_ASSOC)) {
				...
				echo '<td>' . $row['time'] . '</td>
					...
					<td align="left"><a href="trans_receipt_all.php?t_id=' .$row['t_id']. '&time=' .$row['time']. '" onClick="return popup(this, \'trans_receipt\')";><b>' . $row['emp_no'] . '</a></td>
					<td align="left">';
//ccm-rle changed the card_no to equal 2 at this point because that is what we have assigned for non-members whereas 1 is a standard member
				if ($row['card_no'] == 2) { echo "NON-MEMBER"; } 
