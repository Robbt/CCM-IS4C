...
// set_error_handler("pos2_dataError");
	...
	elseif (strlen($entered) == 2) {
		...
		switch ($entered) {
			...
			case "LN":
				...
				break;
//ccm-rle 10-6-2009 added "DP" to do a drop down for departments to select one vs. the department keys alone
                        case "DP":
                                $intAway = 1;
                                maindisplay("department.php");
                                break;
...
//				tender("EC", $_SESSION["runningTotal"] * 100);
				break;
			case "FS":
				boxMsg("EBT tender must specify amount");
				break;
			...
			case "MI":
				...
				// elseif ($_SESSION["CashierNo"] < 51 || $_SESSION["CashierNo"] > 55) {
				//	xboxMsg("cashier ".$_SESSION["CashierNo"]." cannot accept employee charge");
				// }
