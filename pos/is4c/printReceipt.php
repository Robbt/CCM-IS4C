...
// $_SESSION["noreceipt"] = 1; // ccm-rle 9-25-09 tried to set variable to turn of printing of receipts by default while still enabling printing manually but this appears to have no effect on receipt printing. so it's commented out and the line setting it to 0 below is now uncommmented.
...
/* --------------------------------------------------------------
  ...
  ---------------------------------------------------------------- */
	...
	if ($_SESSION["TenderType"] == "MI" and ($_SESSION["receiptToggle"] == 0 or $_SESSION["SCReceipt"] == 0)) $_SESSION["noreceipt"] = 1;	// apbw 2/15/05 SCR
...
// ccm-rle - will need to modify this to equal whatever we choose for non-member  I changed the code because we aren't currently using memberships	
		if (trim($_SESSION["memberID"]) != 2) {			//	mem# 99999 = NON-MEMBER 
//			$receipt .= centerString("Thank You - ".$member)."\n";
		...
		elseif ($arg1 == "suspended") {
			$receipt .= $dashes.centerString("*  T R A N S A C T I O N  S U S P E N D E D  *").$dashes. $option
				     .centerString($ref);
		}
	}
}
...
$_SESSION["noreceipt"] = 1;	// apbw 2/15/05 SCR - 9/25/09 ccm-rle commented out and a variable with noreceipt = 0 set at top to see if it turns of receipt printing while still enabling manual receipt printing
