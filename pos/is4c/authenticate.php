...
if (!function_exists("loginscreen")) include("clientscripts.php");
if (!function_exists("memberID")) include("prehkeys.php");

// set_error_handler("auth_dataError");
...
if (!is_numeric($password)) header("Location:invalid.php");
elseif ($password > 9999 || $password < 1) header("Location:invalid.php");
else {
	...
	if ($row_g["LoggedIn"] == 0) {
		...
		if ($num_rows_q > 0) {
			...
			setglobalvalue("cashier", $row_q["FirstName"]." ".substr($row_q["LastName"], 0, 1).".");
                        $employee_no = $row_q["emp_no"];

			loadglobalvalues();

			$transno = gettransno($employee_no);
